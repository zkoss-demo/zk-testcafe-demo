...
// workaround for https://github.com/DevExpress/testcafe/issues/2763 
// avoid jumping button due to height changes from loading images
const fixStepbarContentHeight = ClientFunction(() => { document.querySelector('.stepbar-content').style.height = '200px' });
...
test('ZK demo: step bar example test', async t => {
    ...
    await fixStepbarContentHeight(); //call workaround
    
    await t.click(addCarButton)
